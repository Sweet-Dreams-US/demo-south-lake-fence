...
/**
 * Styles & Materials header — the scroll DRIVES the fence-material film (the
 * frames scrub as you scroll; nothing autoplays). It lives in a nested
 * scroll-snap pager, so each scroll locks onto the next material stop — you
 * never end up stranded between two materials.
 */
    ...
    const step = (now: number) => {
      ...
      if (k < 1) requestAnimationFrame(step);
        ...
        const settle = () => {
          ...
          if (++m < 4) requestAnimationFrame(settle);
        };
          ...
          >
            ...
            >
              ...
              <button
                ...
                className="pointer-events-auto mt-4 inline-flex touch-manipulation items-center gap-2 rounded-full bg-gold px-6 py-3 text-sm font-semibold text-ink shadow-lg shadow-black/30 transition-transform active:scale-95"
